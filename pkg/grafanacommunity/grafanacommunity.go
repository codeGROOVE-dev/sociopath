// Package grafanacommunity fetches Grafana Community forum profile data.
package grafanacommunity
...
const platform = "grafanacommunity"

// platformInfo implements profile.Platform for Grafana Community.
...
var usernamePattern = regexp.MustCompile(`(?i)community\.grafana\.com/u/([a-zA-Z0-9_-]+)`)

// Match returns true if the URL is a Grafana Community profile URL.
func Match(urlStr string) bool {
	lower := strings.ToLower(urlStr)
	return strings.Contains(lower, "community.grafana.com/u/") && usernamePattern.MatchString(urlStr)
}

// AuthRequired returns false because Discourse forums are public.
func AuthRequired() bool { return false }

// Client handles Grafana Community requests.
type Client struct {
...
// New creates a Grafana Community client.
...
// Fetch retrieves a Grafana Community profile.
func (c *Client) Fetch(ctx context.Context, urlStr string) (*profile.Profile, error) {
	...
	c.logger.InfoContext(ctx, "fetching grafana community profile", "url", urlStr, "username", username)

	// Use Discourse API
	apiURL := fmt.Sprintf("https://community.grafana.com/u/%s.json", username)
...
func parseProfile(data *discourseUser, url string) *profile.Profile {
	...
	if data.AvatarTemplate != "" {
		...
		if !strings.HasPrefix(avatarURL, "http") {
			avatarURL = "https://community.grafana.com" + avatarURL
		}
