// Package linuxmint fetches Linux Mint Forums profile data.
package linuxmint
...
var profilePattern = regexp.MustCompile(`forums\.linuxmint\.com/memberlist\.php\?mode=viewprofile&u=(\d+)`)

func Match(urlStr string) bool {
	return strings.Contains(urlStr, "forums.linuxmint.com/memberlist.php") && profilePattern.MatchString(urlStr)
}
...
func (c *Client) Fetch(ctx context.Context, urlStr string) (*profile.Profile, error) {
	c.logger.InfoContext(ctx, "fetching linux mint profile", "url", urlStr)
...
func parseProfile(body []byte, profileURL string) (*profile.Profile, error) {
	...
	// phpBB 2.0 profile structure - Extract username from title
	titleRe := regexp.MustCompile(`<title>([^<]+) :: Linux Mint Forums</title>`)
	...
	if matches := avatarRe.FindStringSubmatch(html); len(matches) > 1 {
		...
		if !strings.HasPrefix(p.AvatarURL, "http") {
			p.AvatarURL = "https://forums.linuxmint.com" + p.AvatarURL
		}
	}

	// phpBB uses table-based layout with specific cell classes
