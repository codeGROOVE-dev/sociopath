...
var usernamePattern = regexp.MustCompile(`(?i)sourceforge\.net/u(?:ser(?:s)?)?/(?:username/)?([a-zA-Z0-9_-]+)`)

// Match returns true if the URL is a SourceForge profile URL.
func Match(urlStr string) bool {
	lower := strings.ToLower(urlStr)
	if !strings.Contains(lower, "sourceforge.net") {
		return false
	}
	return usernamePattern.MatchString(urlStr)
}

// AuthRequired returns false because SourceForge profiles are public.
func AuthRequired() bool { return false }

// Client handles SourceForge requests.
type Client struct {
...
// New creates a SourceForge client.
...
var (
	displayNamePattern = regexp.MustCompile(`(?i)<h1[^>]*class="[^"]*project-title[^"]*"[^>]*>([^<]+)</h1>`)
	displayNameAlt     = regexp.MustCompile(`(?i)<div[^>]*class="[^"]*profile-name[^"]*"[^>]*>([^<]+)</div>`)
	avatarPattern      = regexp.MustCompile(`(?i)<img[^>]+class="[^"]*user-photo[^"]*"[^>]+src="([^"]+)"`)
	avatarAlt          = regexp.MustCompile(`(?i)<img[^>]+src="([^"]+)"[^>]+class="[^"]*avatar[^"]*"`)
	joinedPattern      = regexp.MustCompile(`(?i)Member Since[:\s]*([^<]+)<`)
	projectsPattern    = regexp.MustCompile(`(?i)(\d+)\s*Projects?`)
	bioPattern         = regexp.MustCompile(`(?i)<div[^>]*class="[^"]*bio[^"]*"[^>]*>([^<]+)</div>`)
)

// Fetch retrieves a SourceForge profile.
func (c *Client) Fetch(ctx context.Context, urlStr string) (*profile.Profile, error) {
	...
	c.logger.InfoContext(ctx, "fetching sourceforge profile", "url", urlStr, "username", username)
