// Package infoqcn fetches InfoQ China user profile data.
package infoqcn
...
const platform = "infoqcn"

// platformInfo implements profile.Platform for InfoQ China.
...
var usernamePattern = regexp.MustCompile(`(?i)infoq\.cn/u/([a-zA-Z0-9_-]+)`)

// Match returns true if the URL is an InfoQ China profile URL.
func Match(urlStr string) bool {
	lower := strings.ToLower(urlStr)
	if !strings.Contains(lower, "infoq.cn/") {
		return false
	}
	return usernamePattern.MatchString(urlStr)
}

// AuthRequired returns false because InfoQ China profiles are public.
func AuthRequired() bool { return false }

// Client handles InfoQ China requests.
type Client struct {
...
// New creates an InfoQ China client.
...
// Fetch retrieves an InfoQ China profile.
func (c *Client) Fetch(ctx context.Context, urlStr string) (*profile.Profile, error) {
	...
	c.logger.InfoContext(ctx, "fetching infoqcn profile", "url", urlStr, "username", username)

	profileURL := fmt.Sprintf("https://www.infoq.cn/u/%s", username)
...
func parseHTML(body []byte, username, url string) (*profile.Profile, error) {
	...
	if err != nil {
		return nil, fmt.Errorf("failed to parse infoqcn HTML: %w", err)
	}
	...
	extract = func(n *html.Node) {
		if n.Type == html.ElementNode {
			if n.Data == "title" && n.FirstChild != nil {
				...
				if strings.Contains(title, " - InfoQ") {
					parts := strings.Split(title, " - InfoQ")
					if len(parts) > 0 {
			...
			if hasClass(n, "article") || hasClass(n, "article-item") {
...
func extractPost(n *html.Node) profile.Post {
	...
	extract = func(node *html.Node) {
		if node.Type == html.ElementNode {
			...
			if hasClass(node, "content") || hasClass(node, "summary") {
