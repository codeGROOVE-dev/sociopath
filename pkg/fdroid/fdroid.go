// Package fdroid fetches F-Droid Forum user profile data.
package fdroid
...
const platform = "fdroid"

// platformInfo implements profile.Platform for F-Droid Forum.
...
var usernamePattern = regexp.MustCompile(`(?i)forum\.f-droid\.org/u/([a-zA-Z0-9_-]+)`)

// Match returns true if the URL is an F-Droid Forum user profile URL.
func Match(urlStr string) bool {
	lower := strings.ToLower(urlStr)
	return strings.Contains(lower, "forum.f-droid.org/u/") && usernamePattern.MatchString(urlStr)
}

// AuthRequired returns false because F-Droid Forum profiles are public.
func AuthRequired() bool { return false }

// Client handles F-Droid Forum requests.
type Client struct {
...
// New creates an F-Droid Forum client.
...
// Fetch retrieves an F-Droid Forum profile.
func (c *Client) Fetch(ctx context.Context, urlStr string) (*profile.Profile, error) {
	...
	c.logger.InfoContext(ctx, "fetching fdroid forum profile", "url", urlStr, "username", username)

	apiURL := fmt.Sprintf("https://forum.f-droid.org/u/%s.json", username)
	...
	if err := json.Unmarshal(body, &data); err != nil {
		return nil, fmt.Errorf("failed to parse fdroid forum response: %w", err)
	}
