// Package kubernetesdiscuss fetches Kubernetes Discuss forum profile data.
package kubernetesdiscuss
...
const platform = "kubernetesdiscuss"

// platformInfo implements profile.Platform for Kubernetes Discuss.
...
// fetchProfile is the FetchFunc for Kubernetes Discuss profiles.
...
var usernamePattern = regexp.MustCompile(`(?i)discuss\.kubernetes\.io/u/([a-zA-Z0-9_-]+)`)

// Match returns true if the URL is a Kubernetes Discuss profile URL.
func Match(urlStr string) bool {
	lower := strings.ToLower(urlStr)
	return strings.Contains(lower, "discuss.kubernetes.io/u/") && usernamePattern.MatchString(urlStr)
}

// AuthRequired returns false because Discourse forums are public.
func AuthRequired() bool { return false }

// Client handles Kubernetes Discuss requests.
type Client struct {
...
// New creates a Kubernetes Discuss client.
...
// Fetch retrieves a Kubernetes Discuss profile.
func (c *Client) Fetch(ctx context.Context, urlStr string) (*profile.Profile, error) {
	...
	c.logger.InfoContext(ctx, "fetching kubernetes discuss profile", "url", urlStr, "username", username)

	// Use Discourse API
	apiURL := fmt.Sprintf("https://discuss.kubernetes.io/u/%s.json", username)
...
func parseProfile(data *discourseUser, url string) *profile.Profile {
	...
	if data.AvatarTemplate != "" {
		...
		if !strings.HasPrefix(avatarURL, "http") {
			avatarURL = "https://discuss.kubernetes.io" + avatarURL
		}
	...
	// Add post and engagement stats
	if data.PostCount > 0 {
		p.Fields["post_count"] = fmt.Sprintf("%d", data.PostCount)
	}
	if data.TopicCount > 0 {
		p.Fields["topic_count"] = fmt.Sprintf("%d", data.TopicCount)
	}
	if data.LikesGiven > 0 {
		p.Fields["likes_given"] = fmt.Sprintf("%d", data.LikesGiven)
	}
	if data.LikesReceived > 0 {
		p.Fields["likes_received"] = fmt.Sprintf("%d", data.LikesReceived)
	}
	if data.DaysVisited > 0 {
		p.Fields["days_visited"] = fmt.Sprintf("%d", data.DaysVisited)
	}
