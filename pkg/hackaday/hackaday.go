...
var usernamePattern = regexp.MustCompile(`(?i)hackaday\.io/([a-zA-Z0-9_.-]+)`)

// Match returns true if the URL is a Hackaday.io profile URL.
func Match(urlStr string) bool {
	lower := strings.ToLower(urlStr)
	if !strings.Contains(lower, "hackaday.io") {
		return false
	}
	// Exclude non-profile paths
	excluded := []string{"/project/", "/projects/", "/contest/", "/list/", "/page/"}
...
// AuthRequired returns false because Hackaday.io profiles are public.
func AuthRequired() bool { return false }

// Client handles Hackaday requests.
type Client struct {
...
var (
	displayNamePattern = regexp.MustCompile(`(?i)<h1[^>]*class="[^"]*hacker-name[^"]*"[^>]*>([^<]+)</h1>`)
	...
	websitePattern     = regexp.MustCompile(`(?i)<a[^>]+href="(https?://[^"]+)"[^>]*rel="[^"]*nofollow[^"]*"`)
)

// Fetch retrieves a Hackaday.io profile.
func (c *Client) Fetch(ctx context.Context, urlStr string) (*profile.Profile, error) {
	...
	c.logger.InfoContext(ctx, "fetching hackaday profile", "url", urlStr, "username", username)

	profileURL := "https://hackaday.io/" + username
...
func parseProfile(html, username, url string) *profile.Profile {
	...
	if m := bioMeta.FindStringSubmatch(html); len(m) > 1 {
		...
		if !strings.Contains(bio, "Hackaday.io") && len(bio) > 20 {
			prof.Bio = bio
		}
	}
	if prof.Bio == "" {
	...
	if m := websitePattern.FindStringSubmatch(html); len(m) > 1 {
		website := m[1]
		if !strings.Contains(website, "hackaday") {
